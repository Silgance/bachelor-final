    ...
    while (true)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            if (estimator.m_fast_lio_instance != nullptr)
            {
                ...
                lock_lio(estimator);
                t_s.tic();
                double camera_LiDAR_tim_diff = img_msg->header.stamp.toSec() + g_camera_lidar_queue.m_camera_imu_td - g_lio_state.last_update_time; //屁用没有
            ...
            int skip_imu = 0;   //屁用没有
            for (auto &imu_msg : measurement.first)//预积分
            {
                double t = imu_msg->header.stamp.toSec();   //imu时间
                double img_t = img_msg->header.stamp.toSec() + estimator.td;    //img时间
